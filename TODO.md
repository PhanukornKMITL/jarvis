...
## ความเป็นธรรมชาติ (คุยกันเมื่อ 2026-09-25 จาก log 79 ครั้งใน `work/dataset/`)

เรียงตามผลต่อความรู้สึก ทำทีละข้อและวัดผลก่อนไปข้อถัดไป

1. **สำนวนคำตอบ:** ทดสอบ prompt เดิมกับ prompt ใหม่ด้วยประโยคจริงจาก log ("โอเค",
   "ชวนคุยหน่อย", "คุณคิดยังไง", "น่าสนใจนะ แต่น่าเบื่อ") เทียบกันทีละคู่ก่อนเปลี่ยน
   - "โอเค" / "ขอบคุณ" / "ได้" ตอบ "ครับ" ด้วยกฎ ไม่ส่ง Qwen (ตอนนี้ได้ประโยคยาวกลับมา)
   - ห้ามและตัดทิ้ง "ขออภัยที่ทำให้คุณสับสน", "ในฐานะ AI ผมไม่มีความคิด"
   - ให้ความเห็นได้ในฐานะ JARVIS
   - ตัวอย่างบทสนทนา 4–5 แบบที่ต่างกัน (ตัวอย่างเดียว Qwen ลอกไปตอบทุกครั้ง)
2. **เก็บเวลาแต่ละขั้นลง log:** พูดจบ → ถอดเสียง → ตรวจเสียงเจ้าของ → intent → เสียงแรก
   ตอนนี้มีแต่ค่าประมาณ (สั่งไฟ ~1.8 วิ, chat filler ~2 วิ คำตอบ ~3 วิ)
3. **ตัดสินว่าพูดจบจากความหมาย:** เงียบ 0.3 วิ ให้ base ถอดสิ่งที่ได้ยิน ถ้าเป็นคำสั่งครบ
   (ปิดไฟที, กี่โมง) ตอบเลย คำถามทั่วไปยังรอ 0.8 วิ แยกตัวตัดสินไว้ให้เปลี่ยนเป็นโมเดลได้ภายหลัง
4. **ตอบรับสั้นๆ ทันที** ("ครับ", "อืม") ปิดได้ด้วย config
5. ✅ (ไม่ต้องแก้: ระบบเช็คเสียงเจ้าของก่อนส่งให้ Thonburian อยู่แล้ว) **ตื่นเองจากเสียงรบกวน:** 21:44–23:04 ของ 2026-09-24 ตื่นเอง ~30 ครั้ง ได้คำหลอนของ
   Whisper ("สินค้า สินค้า", "ติดตามช่อง", ประโยคซ้ำยาว) ระบบจำเสียงบล็อกได้หมด แต่เปลือง
   Thonburian ทุกครั้ง กรองคำหลอนและข้อความซ้ำก่อนส่งให้ตัวถอดเสียงใหญ่
6. ✅ 2026-09-26 **โหมดทำงาน** แบบห้องแล็บ Tony: "จาวิส เริ่มทำงาน" แล้วคุยได้ไม่ต้องเรียกชื่อจนพูด "พอแล้ว"
   หรือเงียบ ~10 นาที เฉพาะเสียงเจ้าของ

## พรุ่งนี้ (2026-09-26): ESP32 จริง

1. ต่อ ESP32 จริงแทนไฟจำลอง (เอา `light` ออกจาก `[run] fake_devices`)
2. ทดลองส่งเสียง JARVIS ออกลำโพงที่ ESP32 (เช่นโมดูลขยายเสียง I2S อย่าง MAX98357) แทนลำโพง Mac
   - ถ้าลำโพงอยู่ห่างจากไมค์ ระบบหักเสียงสะท้อนของ barge-in ต้องวัดค่าใหม่ (ตอนนี้จูนกับหูฟัง)
   - เสียงที่ส่งผ่าน Wi-Fi มีดีเลย์ ต้องวัดว่าเพิ่มกี่วินาที
3. ✅ 2026-09-26 **ตั้งปลุก** (แยกจากเตือนความจำ): "ปลุกผมตีห้าครึ่ง" / "ปลุกทุกวันธรรมดา 7 โมง"
   - ถึงเวลาเล่นเสียงปลุกและพูดซ้ำจนกว่าจะตอบ ("ตื่นแล้ว", "หยุด") ไม่ใช่พูดครั้งเดียวแบบเตือนความจำ
   - "ขอนอนอีก 10 นาที" = เลื่อนปลุก (snooze)
   - ตื่นแล้วทักอรุณสวัสดิ์พร้อมอากาศวันนี้และนัดที่ตั้งไว้ (ต่อกับเรื่องทักทายตอนเช้าใน VISION)
   - Mac ต้องไม่ sleep ตอนกลางคืน ไม่งั้นปลุกไม่ดัง (ดู `caffeinate` หรือตั้งค่า Energy)

## ทำแล้ว 2026-09-26 (ไม่ต้องใช้ฮาร์ดแวร์)

- ปลุกดังจนกว่าจะตอบ, "ขอนอนอีก 10 นาที", อรุณสวัสดิ์พร้อมอากาศและนัดวันนี้, Mac ไม่ sleep ตอน JARVIS รัน
- ตอบรับคำสั่งไฟหลายแบบ เรียกชื่อ/บอส/เจ้านายสลับ และเสียงคลิก (VISION)
- ทักอรุณสวัสดิ์ตอนคุยครั้งแรกของเช้า (5–11 โมง), ฝันดีพร้อมเสนอปิดไฟถ้ายังเปิด
- โหมดทำงาน: "จาวิส เริ่มทำงาน" คุยได้ไม่ต้องเรียกชื่อจน "เลิกทำงาน" หรือเงียบ 10 นาที
